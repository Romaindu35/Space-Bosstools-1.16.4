...
@OnlyIn(Dist.CLIENT)
public class Tier1SpaceStationGuiGuiWindow extends ContainerScreen<Tier1SpaceStationGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	public Tier1SpaceStationGuiGuiWindow(Tier1SpaceStationGuiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 102, this.guiTop + 69, 55, 20, new StringTextComponent("CREATE"), e -> {
			BossToolsMod.PACKET_HANDLER.sendToServer(new Tier1SpaceStationGuiGui.ButtonPressedMessage(0, x, y, z));
			Tier1SpaceStationGuiGui.handleButtonAction(entity, 0, x, y, z);
		}));
		this.addButton(new Button(this.guiLeft + 9, this.guiTop + 14, 68, 20, new StringTextComponent("Planets List"), e -> {
			BossToolsMod.PACKET_HANDLER.sendToServer(new Tier1SpaceStationGuiGui.ButtonPressedMessage(1, x, y, z));
			Tier1SpaceStationGuiGui.handleButtonAction(entity, 1, x, y, z);
		}));
	}
}
