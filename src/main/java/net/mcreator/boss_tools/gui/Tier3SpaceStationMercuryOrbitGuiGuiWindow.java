...
@OnlyIn(Dist.CLIENT)
public class Tier3SpaceStationMercuryOrbitGuiGuiWindow extends ContainerScreen<Tier3SpaceStationMercuryOrbitGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	public Tier3SpaceStationMercuryOrbitGuiGuiWindow(Tier3SpaceStationMercuryOrbitGuiGui.GuiContainerMod container, PlayerInventory inventory,
			ITextComponent text) {
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack ms, float par1, int par2, int par3) {
		...
		this.blit(ms, this.guiLeft + 56, this.guiTop + 64, 0, 0, 32, 31, 32, 31);
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 102, this.guiTop + 69, 55, 20, new StringTextComponent("CREATE"), e -> {
			BossToolsMod.PACKET_HANDLER.sendToServer(new Tier3SpaceStationMercuryOrbitGuiGui.ButtonPressedMessage(0, x, y, z));
			Tier3SpaceStationMercuryOrbitGuiGui.handleButtonAction(entity, 0, x, y, z);
		}));
		this.addButton(new Button(this.guiLeft + 9, this.guiTop + 14, 68, 20, new StringTextComponent("Planets List"), e -> {
			BossToolsMod.PACKET_HANDLER.sendToServer(new Tier3SpaceStationMercuryOrbitGuiGui.ButtonPressedMessage(1, x, y, z));
			Tier3SpaceStationMercuryOrbitGuiGui.handleButtonAction(entity, 1, x, y, z);
		}));
	}
}
