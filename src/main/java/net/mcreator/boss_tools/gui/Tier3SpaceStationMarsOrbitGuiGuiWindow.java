...
@OnlyIn(Dist.CLIENT)
public class Tier3SpaceStationMarsOrbitGuiGuiWindow extends ContainerScreen<Tier3SpaceStationMarsOrbitGuiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	public Tier3SpaceStationMarsOrbitGuiGuiWindow(Tier3SpaceStationMarsOrbitGuiGui.GuiContainerMod container, PlayerInventory inventory,
			ITextComponent text) {
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 102, this.guiTop + 69, 55, 20, new StringTextComponent("CREATE"), e -> {
			BossToolsMod.PACKET_HANDLER.sendToServer(new Tier3SpaceStationMarsOrbitGuiGui.ButtonPressedMessage(0, x, y, z));
			Tier3SpaceStationMarsOrbitGuiGui.handleButtonAction(entity, 0, x, y, z);
		}));
		this.addButton(new Button(this.guiLeft + 9, this.guiTop + 14, 68, 20, new StringTextComponent("Planets List"), e -> {
			BossToolsMod.PACKET_HANDLER.sendToServer(new Tier3SpaceStationMarsOrbitGuiGui.ButtonPressedMessage(1, x, y, z));
			Tier3SpaceStationMarsOrbitGuiGui.handleButtonAction(entity, 1, x, y, z);
		}));
	}
}
